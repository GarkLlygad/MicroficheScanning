...
allItems = []
for iSearch in search_items(f'uploader:wjrussell829@gmail.com micro_review:rescan'):
        allItems.append(iSearch['identifier'])
itemCountTotal = len(allItems)
print('wesley')
print(itemCountTotal)
print(allItems)
with open(f"/home/gark/MicroficheScanning/scanners/wesley.html") as fp:
    soup = BeautifulSoup(fp, 'html.parser')
    soup.find(id=f"wesleyRescansTotal").string = str(itemCountTotal)
    soup.find(id=f"wesleyRescansID").clear()
    for indexItems, b in enumerate(allItems):
        newListTag = soup.new_tag("li")
        newAnchorTag = soup.new_tag("a", href=f"https://archive.org/details/{str(b)}")
        newAnchorTag.string = str(b)
        newListTag.append(newAnchorTag)
        soup.find(id=f"wesleyRescansID").insert(1, newListTag)
with open(f"/home/gark/MicroficheScanning/scanners/wesley.html", "w") as fp:
    fp.write(soup.prettify())
